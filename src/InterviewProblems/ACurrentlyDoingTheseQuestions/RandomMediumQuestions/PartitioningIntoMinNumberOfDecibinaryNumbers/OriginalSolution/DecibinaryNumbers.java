package InterviewProblems.ACurrentlyDoingTheseQuestions.RandomMediumQuestions.PartitioningIntoMinNumberOfDecibinaryNumbers.OriginalSolution;
...
public class DecibinaryNumbers {
    ...
    public int minPartitions(String n) {
        int minNumDecibinaryNumbers = -1;
        for (char number : n.toCharArray()) {
            int digit = Character.getNumericValue(number);
            minNumDecibinaryNumbers = Math.max(minNumDecibinaryNumbers, digit);
        }
        return minNumDecibinaryNumbers;
    }
}
