package InterviewProblems.MediumQuestions.Stacks.CarFleet.WithObjects;
...
public class CarFleet {
    ...
    // source: 106ms sample LeetCode solution: https://leetcode.com/submissions/detail/797253710/
    ...
    public int carFleet(int target, int[] position, int[] speed) {
        List<PositionTimeNode> positionTime = new ArrayList<>();
        for (int i = 0; i < position.length; i++) {
            positionTime.add(new PositionTimeNode(position[i], (double) (target - position[i]) / speed[i]));
        }
        positionTime.sort((a, b) -> b.position - a.position);  // sort by position in descending order.

        Stack<Double> arrivalTimes = new Stack<>();
        for (PositionTimeNode positionTimeNode : positionTime) {
            if (arrivalTimes.isEmpty() || positionTimeNode.arrivalTime > arrivalTimes.peek()) {
                arrivalTimes.push(positionTimeNode.arrivalTime);
            }
        }
...
class PositionTimeNode {

    int position;
    double arrivalTime;

    PositionTimeNode(int initialPosition, double initialArrivalTime) {
