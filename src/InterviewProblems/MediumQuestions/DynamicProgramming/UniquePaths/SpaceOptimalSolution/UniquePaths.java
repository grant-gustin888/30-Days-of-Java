package InterviewProblems.MediumQuestions.DynamicProgramming.UniquePaths.SpaceOptimalSolution;
...
public class UniquePaths {
    ...
    public int uniquePaths(int m, int n) {
        int[] numWays = new int[n];  // store a single row of the grid at a time
        Arrays.fill(numWays, 1);

        for (int rowNum = 1; rowNum < m; rowNum++) {
            for (int columnNum = 1; columnNum < n; columnNum++) {
                numWays[columnNum] += numWays[columnNum - 1];
            }
        }
        return numWays[n - 1];
    }
