package InterviewProblems.MediumQuestions.DynamicProgramming.UniquePaths.RecursiveSolution;
...
public class UniquePaths {
    ...
    private int numPathsFrom(int currentRow, int currentColumn) {
        if (currentRow == targetRow && currentColumn == targetColumn) {
            return 1;
        } else if (currentRow > targetRow || currentColumn > targetColumn) {
            return 0;
        } else {
            return  numPathsFrom(currentRow + 1, currentColumn) +
                    numPathsFrom(currentRow, currentColumn + 1);
        }
    }
}
