package InterviewProblems.MediumQuestions.Arrays.FindPeakElement.OriginalCorrectSolution;
...
public class PeakElement {
    ...
    // while loop condition, missing nums[middleIndex - 1] < nums[middleIndex] && nums[middleIndex] > nums[middleIndex + 1],
    // and startIndex/endIndex moving pointers
    ...
    public int findPeakElement(int[] nums) {
        ...
        while (startIndex + 2 <= endIndex) {
            ...
            if (nums[middleIndex - 1] < nums[middleIndex] && nums[middleIndex] > nums[middleIndex + 1]) {
                ...
                endIndex = middleIndex;  // not middleIndex - 1, because middleIndex might still be the peak.
            } else { // nums[middleIndex - 1] < nums[middleIndex]
    ...
    private int binarySearch(int[] array, int targetElement) {
        int startIndex = 0;
        int endIndex = array.length - 1;
        while (startIndex <= endIndex) {
            int middleIndex = (startIndex + endIndex) / 2;  // could overflow
            if (array[middleIndex] == targetElement) {
                return middleIndex;
            } else if (array[middleIndex] < targetElement) {
                endIndex = middleIndex - 1;
            } else {  // array[middleIndex] > targetElement
                startIndex = middleIndex + 1;
            }
        }
        return -1;  // targetElement not found.
    }
}
