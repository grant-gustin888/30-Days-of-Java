package InterviewProblems.DynamicProgramming.HouseRobber.MemoizationSolution;
...
public class HouseRobber {
    ...
    // --> O(n) because at most O(n) stack frames exist on the call stack.
    ...
    public int rob(int[] nums) {
        return robHelper(nums, 0, nums.length - 1, new int[nums.length]);
    }

    private int robHelper(int[] nums, int startIndex, int endIndex, int[] memo) {
        ...
        } else if (memo[startIndex] != 0) {
            return memo[startIndex];
        } else {  // startIndex + 2 >= endIndex, and memo[startIndex] == 0;
            // either include current house, or skip to next house if it's worth more
            int robCurrentHouse = nums[startIndex] + robHelper(nums, startIndex + MIN_DISTANCE_BETWEEN_HOUSES, endIndex, memo);
            int dontRobCurrentHouse = robHelper(nums, startIndex + 1, endIndex, memo);
            memo[startIndex] = Math.max(robCurrentHouse, dontRobCurrentHouse);
            return memo[startIndex];
        }
    }
}
